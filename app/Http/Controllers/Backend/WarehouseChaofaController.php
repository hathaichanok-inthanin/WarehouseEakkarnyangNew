...
class WarehouseChaofaController extends Controller
{
    public function tyreWarehouseChaofa(Request $request) {
        ...
        return view('backend/warehouseChaofa/tyre/index')->with('NUM_PAGE',$NUM_PAGE)
                                                         ->with('page',$page)
                                                         ->with('products',$products);
    }

    public function createTyreWarehouseChaofa() {
        return view('backend/warehouseChaofa/tyre/create-tyre');
    }

    public function createTyreWarehouseChaofaPost(Request $request) {
        $validator = Validator::make($request->all(), $this->rules_createTyreWarehouseChaofaPost(), $this->messages_createTyreWarehouseChaofaPost());
        if($validator->passes()) {
            $product = $request->all();
            $product = TyreWarehouseChaofa::create($product);
    ...
    public function addStockTyre(Request $request) {
        ...
        $amount = TyreWarehouseChaofa::findOrFail($id);
        $amount = TyreWarehouseChaofa::where('id',$id)->value('amount');
        $amount += $amountadd;
        $amount = TyreWarehouseChaofa::where('id',$id)->update(['amount' =>  $amount]);        
        return redirect()->action('App\Http\Controllers\Backend\WarehouseChaofaController@tyreWarehouseChaofa');
    }
    ...
    public function deleteTyre($id){
        TyreWarehouseChaofa::destroy($id);
        return redirect()->action('App\Http\Controllers\Backend\WarehouseChaofaController@tyreWarehouseChaofa');
    }

    public function editTyre($id){
      $product = TyreWarehouseChaofa::findOrFail($id);
      return view('/backend/warehouseChaofa/tyre/edit')->with('product', $product);
    }
    ...
    public function rules_createTyreWarehouseChaofaPost() {
        return [
            'size' => 'required',
            'amount' => 'required',
            'year' => 'required',
    ...
    public function messages_createTyreWarehouseChaofaPost() {
        return [
            'size.required' => 'กรุณากรอกขนาดยางรถยนต์',
            'amount.required' => 'กรุณากรอกจำนวนที่มีในสต๊อก',
            'year.required' => 'กรุณากรอกปีผลิต',
